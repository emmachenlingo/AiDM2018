...
#calculate model parameters: mean rating over the training set:
    ...
    for x in range(1,Nuser+1):
        if x in items:
            mar[x-1] = np.mean(train[train[:,0]==x, 2])
        else:
            mar[x-1] = gmr

#calculate 
#apply the model to the train set:
    rmse_train[fold]=np.sqrt(np.mean((train[:,2]-mar[train[:,0]-1])**2))
    mae_train[fold]=np.mean(np.abs(train[:,2]-mar[train[:,0]-1]))
#apply the model to the test set:
    rmse_test[fold]=np.sqrt(np.mean((test[:,2]-mar[test[:,0]-1])**2))
    mae_test[fold]=np.mean(np.abs(test[:,2]-mar[test[:,0]-1]))
